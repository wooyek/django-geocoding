...
version = get_version("src", "django_geocoding", "__init__.py")
...
setup(
    name='django-geocoding',
    version=version,
    description="""Abstract base models and mixins to store address and GPS coordinates info""",
    long_description=readme + '\n\n' + history,
    author="Janusz Skonieczny",
    author_email='js+pypi@bravelabs.pl',
    url='https://github.com/wooyek/django-geocoding',
    packages=find_packages('src', exclude=["*.tests", "*.tests.*", "tests.*", "tests"]),
    package_dir={'': 'src'},
    py_modules=[splitext(basename(path))[0] for path in glob('src/*.py')],
    entry_points={
        'console_scripts': [
            'django_geocoding=django_geocoding.cli:main'
        ]
    },
